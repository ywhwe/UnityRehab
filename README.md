...
*25.12.05.*
1. 타임 스케일을 1.5f로 변경
2. 주사위가 체공 중일 때는 +y방향 힘을 기존의 0.3배만큼만 받도록 설정 -> 굴리기 버튼 연타시 카메라 앵글 바깥으로 나가지 않도록 하기 위함
   2-1. 주사위가 앵글 밖으로 나가지 않도록 하는 투명벽의 위치 및 범위 조정
---------------------------------------------------------------------------------------------------------------------------------
25.12.10.
주사위를 던진 후 일정 시간동안 화면 앞에 윗면을 보인 후 다시 바닥으로 돌아가는 기능 구현 중...
새 스크립트와 함수들은 일부 완성했으나 주사위 윗면을 감지한 후 현재 윗면이 주사위의 어느 방향인지를 갱신하는 방법과
만들어진 스크립트를 어느 오브젝트에 넣어야 할지 고려 중.
종료 화면 출력 시 주사위를 던지는 버튼을 비활성화 하는 라인 추가(로컬에만 존재 GameManager.cs line76-79)
